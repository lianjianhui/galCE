...
class gasAcc:
 ...
 def mw_multi_burst(self, acc_initial, tau_initial, acc_secular, acc_second, tau_second, acc_post, sfe_initial, tau_sfe, sfe_secular, sfe_second, sfe_postburst, time0=2, time1=7, dt=1):
  ...
  Parameters:
   ...
   dt: duration of the second gas accretion phase.
  '''
 ...
 def exp_single(self, acc_initial, tau_initial, sfe_initial): 
  """
  This method defines simple gas accretion history with accretion rate declines exponentially until today.
  """
  t = np.arange(0,self.ageUni,self.twidth)
  self.accH = acc_initial*exp(-t/tau_initial)

  #assume constant SFL coefficient
  self.sfeH = np.zeros(len(t))
  self.sfeH[:] = sfe_initial

 def exp_double(self, acc_initial, tau_initial, sfe_initial, acc_second, tau_second, sfe_second, time0):
  """
  Double exponential declining gas accretion history. 
  """
  t = np.arange(0,self.ageUni,self.twidth)
  self.accH = np.zeros(len(t)) #Gas accretion hitory
  self.sfeH = np.zeros(len(t)) #Evolutionary history of star formation efficiency

  #Initial phase of gas accretion and star formation
  id0 =  t<=time0
  self.accH[id0] = acc_initial*exp(-1*(t[id0])/tau_initial)
  self.sfeH[id0] = sfe_initial 

  #Second phase of gas accretion and star formation
  id1 =  t>time0
  self.accH[id1] = acc_second*exp(-1*(t[id1]-time0)/tau_second)
  self.sfeH[id1] = sfe_second
